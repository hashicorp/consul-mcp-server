...
import (
	"context"
	"fmt"
	"github.com/hashicorp/consul-mcp-server/pkg/resources"
	"io"
	stdlog "log"
	"net/http"
	"os"
	...
	"github.com/hashicorp/consul-mcp-server/pkg/tools"
...
var (
	rootCmd = &cobra.Command{
		Use:     "consul-mcp-server",
		Short:   "Consul MCP Server",
		Long:    `A Consul MCP server that handles various tools and resources.`,
...
func serverInit(ctx context.Context, hcServer *server.MCPServer, logger *log.Logger) error {
	...
	_, _ = fmt.Fprintf(os.Stderr, "Consul MCP Server running on stdio\n")
...
func streamableHTTPServerInit(ctx context.Context, hcServer *server.MCPServer, logger *log.Logger, host string, port string, endpointPath string) error {
	...
	mux.HandleFunc("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		response := fmt.Sprintf(`{"status":"ok","service":"consul-mcp-server","transport":"streamable-http","endpoint":"%s"}`, endpointPath)

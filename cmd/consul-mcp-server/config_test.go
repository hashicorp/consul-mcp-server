...
func TestGetEndpointPath(t *testing.T) {
	...
	os.Setenv("MCP_ENDPOINT", "/consul")
	path = getEndpointPath(nil)
	assert.Equal(t, "/consul", path, "Endpoint path should be the value of MCP_ENDPOINT when it is set")

	// Test case: Custom endpoint path value
	os.Setenv("MCP_ENDPOINT", "/api/v1/consul-mcp")
	path = getEndpointPath(nil)
	assert.Equal(t, "/api/v1/consul-mcp", path, "Endpoint path should be the custom value set in MCP_ENDPOINT")

...
# Build flags
LDFLAGS=-ldflags="-s -w -X consul-mcp-server/version.GitCommit=$(shell git rev-parse HEAD) -X consul-mcp-server/version.BuildDate=$(shell git show --no-show-signature -s --format=%cd --date=format:"%Y-%m-%dT%H:%M:%SZ" HEAD)"
...
# Run security tests
test-security:
	$(GO) test ./cmd/consul-mcp-server -v -run "TestIs|TestLoad|TestSecurity|TestOptions"

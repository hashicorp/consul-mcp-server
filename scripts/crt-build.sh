...
# The crt-build script is used to determine build metadata and create consul-mcp-server builds.
...
function build() {
  ...
  # Build our ldflags
  msg="--> Building consul-mcp-server revision $revision, built $build_date"
  ...
  ldflags="${ldflags} -X consul-mcp-server/version.GitCommit=$revision -X consul-mcp-server/version.BuildDate=$build_date"

  if [[ ${BASE_VERSION+x} ]]; then
    msg="${msg}, base version ${BASE_VERSION}"
    ldflags="${ldflags} -X consul-mcp-server/version.Version=$BASE_VERSION"
  fi

  if [[ ${PRERELEASE_VERSION+x} ]]; then
    msg="${msg}, prerelease ${PRERELEASE_VERSION}"
    ldflags="${ldflags} -X consul-mcp-server/version.VersionPrerelease=$PRERELEASE_VERSION"
  fi

  if [[ ${METADATA_VERSION+x} ]]; then
    msg="${msg}, metadata ${METADATA_VERSION}"
    ldflags="${ldflags} -X consul-mcp-server/version.VersionMetadata=$METADATA_VERSION"
  fi

  # Build consul-mcp-server
  # Always use CGO_ENABLED=0 to ensure a statically linked binary is built
  echo "$msg"
  CGO_ENABLED=0 go build -o "$BIN_PATH" -tags "$GO_TAGS" -ldflags "$ldflags" -trimpath -buildvcs=false ./cmd/consul-mcp-server
}

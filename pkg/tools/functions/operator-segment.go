...
func GetOperatorSegmentTool(logger *log.Logger) server.ServerTool {
	tool := mcp.NewTool("operator_segment",
		mcp.WithDescription("Returns the network segment information for the Consul cluster."),
		mcp.WithTitleAnnotation("Get network segment information from the Consul cluster"),
	...
	return server.ServerTool{
		...
		Handler: func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			return getOperatorSegmentHandler(ctx, request, logger)
		},
	}
}

func getOperatorSegmentHandler(ctx context.Context, request mcp.CallToolRequest, logger *log.Logger) (*mcp.CallToolResult, error) {
	...
	segmentJson := strings.TrimSpace(string(segmentResp))
	return mcp.NewToolResultText(segmentJson), nil
}

...
func GetOperatorUsageTool(logger *log.Logger) server.ServerTool {
	tool := mcp.NewTool("operator_usage",
		mcp.WithDescription("Returns usage information and metrics for the Consul cluster."),
		mcp.WithTitleAnnotation("Get usage information and metrics from the Consul cluster"),
	...
	return server.ServerTool{
		...
		Handler: func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			return getOperatorUsageHandler(ctx, request, logger)
		},
	}
}

func getOperatorUsageHandler(ctx context.Context, request mcp.CallToolRequest, logger *log.Logger) (*mcp.CallToolResult, error) {
	...
	// convert usageResp i.e. bytes[] to text
	usageJson := strings.TrimSpace(string(usageResp))
	return mcp.NewToolResultText(usageJson), nil
}
